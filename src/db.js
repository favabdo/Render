...
const TABLE_NAME = process.env.DB_TABLE_NAME || 'NileChat_byA';
...
const config = {
  ...
  pool: {
    max: 10,
    min: 1, // فضّينا كونكشن واحد سخن دايمًا، عشان أول رسالة بعد فترة سكون معملهاش
...
function getPool() {
  if (!poolPromise) {
    poolPromise = new sql.ConnectionPool(config)
      .connect()
      .then((pool) => {
        console.log('✅ متصل بنجاح بقاعدة بيانات SQL Server:', process.env.DB_NAME);
        return pool;
      })
      .catch((err) => {
        poolPromise = null;
...
async function ensureTableExists() {
  ...
  await pool.request().query(`
    ...
    END
  `);
  console.log(`✅ الجدول [${TABLE_NAME}] جاهز.`);
}
