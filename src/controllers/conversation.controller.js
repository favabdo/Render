...
async function listConversations(req, res) {
  ...
  // labels_json جاي من الـ SQL كـ نص JSON (أو null لو مفيش ليبلز) — بنحوله هنا
  // لمصفوفة labels عادية عشان الفرونت إند يقدر يعرضها في كارت المحادثة على طول
  // من غير ما يضطر يفتح المحادثة الأول عشان يجيبها
  const withLabels = conversations.map(c => {
    let labels = [];
    if (c.labels_json) {
      try {
